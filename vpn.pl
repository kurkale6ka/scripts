...
my $auth = "$vpn/details";
my $protocol = 'udp';
my $download_url = 'https://downloads.nordcdn.com/configs/archives/servers/ovpn.zip';

# Help
my $help = << "";
vpn [pattern] (or use zsh completion)
--config ...,    -c : config file instead of pattern ($vpn/ovpn_$protocol/...)
--show [pattern] -s : show countries
--batch          -b : no codes with --show
--auth ...       -a : credentials file ($vpn/details)
--download       -d : download config files ($download_url)
...
exec 'wget', $download_url if $download;
